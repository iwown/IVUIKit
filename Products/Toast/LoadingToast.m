...
+ (void)makeToastActivityNoTimeOut {
    [self makeToastActivityWithTimeOut:-1];
}

+ (void)makeToastActivityWithTimeOut:(double)second {
    [self makeToastActivityWithTimeOut:second block:nil];
}
...
/**通过__cacheArr机制管理超时问题
 超时问题：
    程序多个地方交叉调用timeout方法，上一个方法的超时返回会导致本次Loading。比如说在程序A处调用timeout=30，
    此时line为0，在10s后程序A主动结束了Loading，在25s的位置程序B也调用timeout=20；但是在程序B的5秒后，
    也就是整个line30s的位置，程序A的timeout会被调用，会提前结束程序B处开启的Loading，引起程序B处的显示问题
 __cacheArr机制：
    0. 创建全局数组__cacheArr，因为Loading的UI操作程序猿会保证它在主线程执行，否则苹果会报warning，
        一般程序员不会这么刚，所以这里忽略多线程问题.
    1. 利用unix时间戳和delay值做唯一id，因为UI操作少有并行，所以秒级的唯一id应该问题不大，复杂id会增加计算量，
        比如说hash
