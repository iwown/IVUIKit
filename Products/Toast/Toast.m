...
+ (void)makeToastActivity:(id)position {
    ...
    activityView.backgroundColor = [UIColor colorWithRed:0.4f green:0.4f blue:0.4f alpha:0.75f];
...
- (CGPoint)centerPointForPosition:(id)point withToast:(UIView *)toast {
    if([point isKindOfClass:[NSString class]]) {
        if([point caseInsensitiveCompare:CSToastPositionTop] == NSOrderedSame) {
            return CGPointMake([UIScreen mainScreen].bounds.size.width/2, (toast.frame.size.height / 2) + CSToastVerticalPadding);
    ...
    // default to bottom
    return CGPointMake([UIScreen mainScreen].bounds.size.width/2, ([UIScreen mainScreen].bounds.size.height - (toast.frame.size.height / 2)) - CSToastVerticalPadding);
}
...
//    wrapperView.backgroundColor = [[UIColor grayColor] colorWithAlphaComponent:CSToastOpacity];
    ...
    if (title != nil) {
        ...
        CGSize maxSizeTitle = CGSizeMake(([UIScreen mainScreen].bounds.size.width * CSToastMaxWidth) - imageWidth, [UIScreen mainScreen].bounds.size.height * CSToastMaxHeight);
    ...
    if (message != nil) {
        ...
        CGSize maxSizeMessage = CGSizeMake(([UIScreen mainScreen].bounds.size.width * CSToastMaxWidth) - imageWidth, [UIScreen mainScreen].bounds.size.height * CSToastMaxHeight);
