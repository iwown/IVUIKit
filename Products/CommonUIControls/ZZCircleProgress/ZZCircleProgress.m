...
- (void)initialization {
    ...
    NSBundle *resourcesBundle = [NSBundle bundleWithPath:[mainBundle pathForResource:@"yccommon.bundle" ofType:nil]];
...
- (void)setProgress:(CGFloat)progress {
    ...
    timer = [NSTimer scheduledTimerWithTimeInterval:0.005 block:^{
        ...
        //反方向动画
        if (isReverse) {
            if (strongSelf->fakeProgress <= strongSelf->_progress || strongSelf->fakeProgress <= 0.0f) {
            ...
            //正方向动画
            if (strongSelf->fakeProgress >= strongSelf->_progress || strongSelf->fakeProgress >= 1.0f) {
                [strongSelf dealWithLast];
                return;
            } else {
                //进度条动画
        ...
        //数值增加或减少
        if (strongSelf->_animationModel == CircleIncreaseSameTime) {
            strongSelf->fakeProgress += defaultIncreaseValue*sameTimeIncreaseValue;//不同进度动画时间基本相同
        } else {
            strongSelf->fakeProgress += defaultIncreaseValue;//进度越大动画时间越长。
        }
